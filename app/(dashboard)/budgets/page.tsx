...
import { supabase, type Budget as SupabaseBudget } from "@/lib/supabase";
...
export default function BudgetsPage() {
  ...
  const [editingBudget, setEditingBudget] = useState<SupabaseBudget | null>(
    null
  );
  ...
  const getBudgetStatus = (spent: number, budget: number) => {
    const percentage = (spent / budget) * 100;
    if (percentage >= 100)
      return { status: "Over Budget", color: "destructive" as const };
    if (percentage >= 80)
